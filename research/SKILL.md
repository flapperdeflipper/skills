...
4. Save it where the repo already keeps such notes. Match the existing convention. If there is none, use the machine's agent scratchpad — `/share/scratchpad/<agent>/<task>/` on the home-assistant box (see `/share/scratchpad/README.md`), `~/.agents/scratchpad/` elsewhere — and say where.
