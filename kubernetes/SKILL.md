---
name: kubernetes
description: "Diagnoses and fixes Kubernetes failure modes: insecure defaults, resource starvation, network exposure, privilege sprawl, fragile rollouts, API drift. Use when generating or reviewing manifests, Helm, Kustomize, or EKS/GKE/AKS/OpenShift work."
---

# Kubernetes: Failure-Mode Workflow

<!-- Adapted from KubeShark by Lukas Niessen (MIT): https://github.com/LukasNiessen/kubernetes-skill. See ./LICENSE. -->

## Rules

- Non-root, read-only root filesystem, drop ALL capabilities by default.
- Resource requests and limits on every container. No exceptions.
- Liveness and readiness probes on every long-running container.
- NetworkPolicy for every namespace that isn't explicitly open.
- Secrets via ESO, Vault CSI, or Sealed Secrets, never plaintext in manifests or ConfigMaps.
- Prefer least-privilege RBAC. ClusterAdmin bindings require explicit justification.
- Pin image tags to digests for production workloads.
