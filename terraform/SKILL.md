---
name: terraform
description: House conventions plus diagnose-first best practices for Terraform/OpenTofu - modules, tests, CI, scans, and state ops; diagnoses failure mode (identity churn, secrets, blast radius, CI drift, state corruption) with version-aware guards. Use when writing, editing, reviewing, or debugging Terraform code (.tf/.tfvars), modules, or infrastructure-as-code.
license: Apache-2.0
metadata:
...
Diagnose-first guidance for Terraform and OpenTofu. Core file is a workflow; depth lives in references loaded on demand. Merged from [antonbabenko/terraform-skill](https://github.com/antonbabenko/terraform-skill) (Apache-2.0) with house rules below.

## House Rules

Non-negotiable conventions for this setup; everything else in this skill is guidance:
...
Apache License 2.0, © Anton Babenko — see [upstream repo](https://github.com/antonbabenko/terraform-skill) for full terms. The House Rules section is original content in this repo (MIT).
